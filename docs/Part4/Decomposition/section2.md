...
&emsp;&emsp;这个问题的特点在于，我们依旧可以把变量分为$l+1$块，从约束角度看，前$l$个块决策变量均与最后一个块决策变量相关，而前$l$个块决策变量相互之间无关。我们把这种与前$l$个独立的块决策变量相关的变量，称为**复杂变量**(complicated variables)。显然这个问题中，如果没有变量$\boldsymbol{y}$，这个问题也可以分割为独立的$l$个问题单独求解，而不会有任何损失。
